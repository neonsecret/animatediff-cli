    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if not image_finetune:
        unet = UNet3DConditionModel.from_pretrained_2d(
            ...
            motion_module_path="data/models/motion-module/mm_sd_v15.safetensors"
        )
    else:
    ...
    train_dataset = YoutubeTuneAVideoDataset(**train_data)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # print(latents.shape)  # 3 4 8 64 64
            first_frames = latents[:, :, :1, :, :].repeat(1, 1, video_length, 1, 1)
            ...
            if noise_scheduler.config.prediction_type == "epsilon":
                target = noisy_first_frames  # The model predicts the spare part without noise
            ...
            with torch.cuda.amp.autocast(enabled=mixed_precision_training):
                model_predicted_noise = unet(noisy_latents, timesteps, encoder_hidden_states).sample
                loss = F.mse_loss(model_predicted_noise.float(), target.float(), reduction="mean")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./config/train/train_img2img.yaml")
