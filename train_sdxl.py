...
from animatediff.pipelines import AnimationPipelineSDXL
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if not os.path.exists(pretrained_model_path) or not os.listdir(pretrained_model_path):
        StableDiffusionXLPipeline.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0", torch_dtype=torch.float16,
                                                  variant="fp16").save_pretrained(pretrained_model_path)
        gc.collect()
    ...
    # Move models to GPU
    validation_pipeline.vae.to(local_rank)
    validation_pipeline.text_encoder.to(local_rank)
    validation_pipeline.text_encoder_2.to(local_rank)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./config/train/train_sdxl.yaml")
